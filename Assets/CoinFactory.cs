...
public class CoinFactory : MonoBehaviour
{
    ...
    float startTime;
    float coinStartTime;
    [SerializeField] float coinInterval = .5f;
    ...
    public void TakeDamageRPC(ulong objectId, float damage)
    {
        ...
        GetComponent<Wall_Health>().m_CurrentHealth -= damage;

        // Change the UI elements appropriately.
        SetHealthUI();

        // If the current health is at or below zero and it has not yet been registered, call OnDeath.
        if (GetComponent<Wall_Health>().m_CurrentHealth <= 0f && !m_Dead)
        {
            OnDeath();
        }
    }
    ...
    public void FixedUpdate()
    {
        ...
        if(Time.time - coinStartTime > coinInterval)
        {
            AddCoin();
    ...
    private void AddCoin()
    {
        if (GetComponent<NetworkObject>().IsOwner)
            GameObject.FindGameObjectWithTag("MyTank").GetComponent<CoinWallet>().UpdateCoinsServerRPC(1);
    }
}
