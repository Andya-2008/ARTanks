    ...
    void FixedUpdate()
    {
        ...
        if (Laser != null && UpdateSaver == false)
        {
            ...
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hit, MaxLength))//CHANGE THIS IF YOU WANT TO USE LASERRS IN 2D: if (hit.collider != null)
            {
                ...
                if (NetworkManager.Singleton.IsServer && hit.collider.gameObject.tag == "MyTank" || hit.collider.gameObject.tag == "Tank")
                {
                    Debug.Log("Lasering " + hit.collider.gameObject.name); 
                    hit.collider.GetComponent<TankHealth>().TakeDamage(Time.fixedDeltaTime * 30);
                }
            }
            else
            {
