...
namespace Complete
{
    public class Wall_Health : NetworkBehaviour
    {
        ...
        [SerializeField] float timedHealthReduction = 0.02f;
        float startTime;
        ...
        public void TakeDamageRPC(ulong objectId, float damage)
        {
            ...
            GetComponent<Wall_Health>().SetHealthUI();

            // If the current health is at or below zero and it has not yet been registered, call OnDeath.
            if(GetComponent<Wall_Health>().m_CurrentHealth <= 0f && !GetComponent<Wall_Health>().m_Dead)
            {
                GetComponent<Wall_Health>().OnDeath();
            }
        }
